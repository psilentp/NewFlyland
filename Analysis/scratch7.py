...
def plot_single_sweep_intro(trial_num,inc_trains,indx):
    plb.figure(figsize = (7,20))
    ephys_sweep = expl.ts(fly[2,indx,trial_num,'AMsysCh1'][0],-0.8,0.2)
    left_wing = expl.ts(fly[2,indx,trial_num,'LeftWing'][0],-0.8,0.2)
    right_wing = expl.ts(fly[2,indx,trial_num,'RightWing'][0],-0.8,0.2)
    La,Ra = expl.get_wbas(left_wing,right_wing)
    phases = expl.get_phase_trace(left_wing,right_wing)
    stim = expl.expan_transform(expl.ts(fly[2,indx,trial_num,'Xpos'][0],-0.8,0.2))
    
    ###########Stim#########
    ax0 = plb.subplot(4,1,1)
    plb.plot(stim.times,stim,color = 'k',alpha = 0.5)
    ax0.set_ybound(0,180)
    ...
    plb.plot(left_wing.times,La,color = (0.7,0.4,0.2))
    plb.plot(right_wing.times,Ra,color = (0.2,0.4,0.7))

    plb.plot(right_wing.times,La-Ra,color = plb.cm.Paired(0.3))
