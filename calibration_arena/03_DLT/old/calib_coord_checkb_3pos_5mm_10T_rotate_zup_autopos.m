...
% strOpos3=[];
% 
% strOpos1 = input('origin at position 1 [default: C4]: ', 's');
% if isempty(strOpos1)
%     strOpos1 = 'C4'; %deg
% end
% 
% strOpos2 = input('origin at position 2 [default: B5]: ', 's');
% if isempty(strOpos2)
%     strOpos2 = 'B5'; %deg
% end
% 
% strOpos3 = input('origin at position 3 [default: C7]: ', 's');
% if isempty(strOpos3)
%     strOpos3 = 'C7'; %deg
% end
...
% from file
file_pos1 = dir('calib_data_pos1*');
file_pos2 = dir('calib_data_pos2*');
file_pos3 = dir('calib_data_pos3*');

% origins for 3 positions
if file_pos1.name(19)>64
    strOpos1 = file_pos1.name(17:18);
else
    strOpos1 = file_pos1.name(17:19);
end

if file_pos2.name(19)>64
    strOpos2 = file_pos2.name(17:18);
else
    strOpos2 = file_pos2.name(17:19);
end

if file_pos3.name(19)>64
    strOpos3 = file_pos3.name(17:18);
else
    strOpos3 = file_pos3.name(17:19);
end

% get value from characters

if strOpos1(1)<97
    Opos1 = [strOpos1(1)-65,str2num(strOpos1(2:end))-1,0];
else
    Opos1 = [strOpos1(1)-97,str2num(strOpos1(2:end))-1,0];
end
    
if strOpos2(1)<97
    Opos2 = [strOpos2(1)-65,str2num(strOpos2(2:end))-1,0];
else
    Opos2 = [strOpos2(1)-97,str2num(strOpos2(2:end))-1,0];
end
    
if strOpos3(1)<97
    Opos3 = [strOpos3(1)-65,str2num(strOpos3(2:end))-1,0];
else
    Opos3 = [strOpos3(1)-97,str2num(strOpos3(2:end))-1,0];
end
...
%% rotate coord system & switch y and z

Frot(:,1) =   F(:,1);
Frot(:,2) = -(F(:,3) .* cosd(beta) + F(:,2) .* sind(beta)); % !!! switch y and z !!!
Frot(:,3) =   F(:,2) .* cosd(beta) - F(:,3) .* sind(beta); % !!! switch y and z !!!
...
save( 'cali_points_rotated_zup', 'F', 'L', 'L_1', 'L_2', 'L_3' );
...
save( 'DLT_coeff_zup', 'DLT','F','L', 'L_1', 'L_2', 'L_3');
